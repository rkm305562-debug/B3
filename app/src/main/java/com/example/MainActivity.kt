...
import com.example.zainqhchat.ui.screens.AuthPopupDialog
...
fun ZainQHChatNavHost(
    ...
    NavHost(
        ...
        composable("splash") {
            if (connectionState != ConnectionState.CONNECTED) {
                ...
                SplashScreen(
                    ...
                    onSplashFinished = {
                        // التصفح مفتوح للجميع دائمًا بعد شاشة البداية — لا
                        // يُطلب تسجيل الدخول هنا إطلاقًا؛ ذلك يحدث فقط لاحقًا
                        // وعند محاولة فتح قسم يتطلب حسابًا (عبر requireAuth).
        ...
        // متاحة للجميع فورًا (زائر أو مسجّل) — لا حجب هنا؛ الحجب فقط عند
        // الدخول إلى قسم بعينه يحتاج حسابًا (عبر requireAuth).
        composable("home") {
            val currentUser = currentUserState

            LaunchedEffect(Unit) {
                if (currentUser == null) authViewModel.checkSession()
            }
        ...
        composable("online_users") {
            ...
            OnlineUsersScreen(
                ...
                onStartPrivateChat = { targetUserId, targetUserName ->
                    requireAuth { navController.navigate("chat/$targetUserId/$targetUserName") }
                },
                onBackClick = { navController.popBackStack() }
            )
        }
    ...
    // نافذة التسجيل/الدخول الزجاجية المنبثقة — تظهر فقط عند استدعاء
    // requireAuth أعلاه (أي عند محاولة زائر الدخول إلى قسم يتطلب حسابًا).
    if (showAuthPopup) {
        AuthPopupDialog(
            authViewModel = authViewModel,
            onDismiss = {
